...
export default function Page() {
  ...
  const handleSetValue = async () => {
    ...
    if (!inputValue.trim()) {setError('Value tidak boleh kosong');return;}
    ...
    if (numericValue <= 0) {
      setError('Value tidak boleh kurang dari atau sama dengan 0');
      return;
    }
  ...
  if (!mounted) return <p className="text-lg">Loading...</p>;
    ...
    <main className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex items-center justify-center text-white">
      <div className="w-full max-w-md bg-gray-900 border border-gray-700 rounded-xl p-6 space-y-6 shadow-lg">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-bold">Full Stack dApp (Avalanche)</h1>
          <p className="text-sm text-gray-400">Day 5 – Avalanche</p>
        </div>
        {/* WALLET STATUS */}
